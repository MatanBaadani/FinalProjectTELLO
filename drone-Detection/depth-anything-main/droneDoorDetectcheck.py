...
def detect_open_door(depth_resized, uppersize):
    # Threshold the depth map to isolate potential door areas
    _, thresh = cv2.threshold(depth_resized, 30, 255, cv2.THRESH_BINARY_INV)
...
# pid fucntion:
def center_door_with_pid(me, depth_anything, transform, DEVICE, input_width, input_height, yaw_PID, pid_duration=20,
                         upper_bound=40000):
    """
    Uses PID control to center the drone on the detected door.
    ...
    # Use PID to center the detected door
...
# rotate to the first door that been detected function:
...
def center_target_with_pid(me, yaw_PID, pid_duration=20, encodeListKnown=None, targets=None):
    """
    Uses PID control to center the drone on the detected target (e.g., a face).

    Parameters:
    - me: The drone object.
    - yaw_PID: The PID controller object for yaw adjustments.
    - pid_duration: The maximum duration (in seconds) to run the PID loop.
    - encodeListKnown: List of known face encodings.
    ...
    while time.time() - pid_start_time < pid_duration:
        ...
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            if matches[best_match_index]:
                ...
                # Use PID to adjust the yaw to center the face
                xVal = int(yaw_PID.update(x_center))
        ...
        # Display the camera feed with face recognition
        cv2.imshow("Drone Camera", img_rgb)
...
pid_start_time = time.time()
forward_start_time = None  # To track the time when the last detection was seen
no_detection_duration = 0  # Duration of no detection

# Move forward while checking for door detection
while True:
    img = me.get_frame_read().frame
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    ...
    # Check for door detection
    detected_door = detect_open_door(depth_resized, 40000)

    if detected_door:
        forward_start_time = time.time()  # Reset the timer
        no_detection_duration = 0
        me.send_rc_control(0, 20, 0, 0)  # Move forward at a constant speed
    else:
        if forward_start_time:
            no_detection_duration = time.time() - forward_start_time
            if no_detection_duration >= 2.0:  # No detection for 1 second
...
center_target_with_pid(me, yaw_PID, pid_duration=20, encodeListKnown=encodeListKnown, targets=targets)
