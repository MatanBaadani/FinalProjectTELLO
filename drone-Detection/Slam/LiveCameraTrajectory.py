...
intrinsic =np.array([[1.40366467e+03, 0.00000000e+00, 6.69703671e+02],
                     [0.00000000e+00, 1.63142182e+03, 5.95433809e+02],
...
# Plot 2D feature points (combined into a single plot)
plt.figure(figsize=(10, 5))
if 'q1' in locals() and 'q2' in locals():  # Check if q1 and q2 are defined
    all_points = np.vstack((q1, q2))  # Combine q1 and q2
    plt.scatter(all_points[:, 0], all_points[:, 1], color='blue', label='Feature Points')
    ...
    print("Feature points q1 and q2 are not available for plotting.")

# Plot estimated path with large limits
plt.figure(figsize=(10, 5))
plt.plot(estimated_path[:, 0], estimated_path[:, 1], marker='o', color='green', label='Estimated Path')
plt.title('Estimated Path with Large Limits')
plt.xlabel('X')
plt.ylabel('Y')
